...
async function createPropelStateDb(
  root: string,
  options: {
    auditRows?: Array<{ createdAt: string; kind: string; payload: string }>;
    sessionRows?: Array<{ createdAt: string; itemType: string; payload: string }>;
  },
): Promise<void> {
  ...
  const statements = [
    ...
    "CREATE TABLE turn_items (id TEXT PRIMARY KEY, turn_id TEXT NOT NULL, session_id TEXT NOT NULL, item_type TEXT NOT NULL, payload_json TEXT NOT NULL, created_at TEXT NOT NULL);",
    ...
    ...sessionRows.map((row, index) => {
      ...
      return `INSERT INTO turn_items (id, turn_id, session_id, item_type, payload_json, created_at) VALUES ('item-${index + 1}', 'turn-${index + 1}', 'session-${index + 1}', '${row.itemType}', '${payload}', '${row.createdAt}');`;
    }),
  ].join(" ");
...
test("loadPropelUsage reads session provider responses from ~/.propel/state.sqlite3 and deduplicates matching audit events", async () => {
  const root = await mkdtemp(join(tmpdir(), "codegraph-propel-session-"));
  ...
  try {
    ...
    await createPropelStateDb(root, {
      auditRows: [
        {
          createdAt: "1773223200",
          kind: "turn.completed",
          payload: JSON.stringify({
            model: "gpt-5-20260301",
            provider: "openai",
            provider_response_id: "resp_shared",
            usage: {
              input_tokens: 120,
              cached_input_tokens: 20,
              output_tokens: 30,
      ...
      sessionRows: [
        ...
        {
          createdAt: "1773225000",
          itemType: "provider_response",
          payload: JSON.stringify({
            model: "claude-sonnet-4-5",
            provider: "anthropic",
            provider_response_id: "resp_session_only",
            usage: {
              input_tokens: 50,
              cache_read_input_tokens: 10,
              output_tokens: 20,
    ...
    assert.equal(summary.insights.mostUsedModel?.name, "gpt-5");
    assert.equal(summary.insights.latestModel?.name, "claude-sonnet-4-5");
    assert.equal(summary.stats.linesScanned, 4);
    assert.equal(summary.stats.badLines, 1);
    assert.equal(summary.stats.eventsConsumed, 2);
  } finally {
    await rm(root, { recursive: true, force: true });
